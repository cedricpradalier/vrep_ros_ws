...
class ModelPrediction:
    def __init__(self):
        ...
        exec ("self.state_sub = rospy.Subscriber('~state', %s , self.state_cb)" % self.state_type[1])
        exec ("self.command_sub = rospy.Subscriber('~command', %s, self.command_cb)" % self.command_type[1])
        self.pub = rospy.Publisher("~prediction",Float64)
